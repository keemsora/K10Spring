...
public class TicketDAO {
	...
	/*
	Spring-JDBC를 사용하기 위한 멤버변수와 setter()
	트랜잭션 처리를 위한 매니저클래스의 멤버변수와 setter()
	*/
	...
	/*
	servlet-context.xml에서 빈을 생성할 때 setter를 사용한다.
	*/
		...
		*/
		try {
			...
			*/
			...
			/*
			티켓 구매갯수가 5장 이하인 경우 정상처리되어 모든 작업이 commit된다.
			*/
			System.out.println("카드결제와 티켓 구매 모두 정상 처리되었습니다.");
			transactionManager.commit(status);
		}
		catch(Exception e) {
			/*
			티켓 구매갯수가 5장을 초과하는 경우에는 예외가 발생하여
			모든 작업이 rollback된다.
			*/
			System.out.println("제약조건을 위배하여 카드 결제와 티켓 구매 모두 취소되었습니다.");
